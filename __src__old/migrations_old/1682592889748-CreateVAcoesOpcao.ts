        ...
        from v_acao_brasil vab 
            left join  
                (select vc.acao, 
                    ...
                    where delta between 0.001 and 0.1  group by vc.acao ) d10 on vab.ticker = d10.acao
            left join 
                (select vc.acao, 
                    max(vc.percentual_lucro) percentual,
                    min(vc.diferenca_percentual) diferenca_percentual from v_calls vc 
                    where delta between 0.001 and 0.2 group by vc.acao ) d20 on vab.ticker = d20.acao
            left join 
                (select vc.acao, 
                    max(vc.percentual_lucro) percentual,
                    min(vc.diferenca_percentual) diferenca_percentual from v_calls vc 
                    where delta between 0.001 and 0.3  group by vc.acao ) d30 on  vab.ticker = d30.acao
            left join			
                (select vc.acao, 
                    max(vc.percentual_lucro) percentual,
                    min(vc.diferenca_percentual) diferenca_percentual from v_calls vc 
                    where delta between 0.001 and 0.4  group by vc.acao ) d40 on  vab.ticker = d40.acao
            left join
                (select vc.acao, 
                    max(vc.percentual_lucro) percentual,
                    min(vc.diferenca_percentual) diferenca_percentual from v_calls vc 
                    where delta between 0.001 and 0.5  group by vc.acao ) d50 on  vab.ticker = d50.acao
`);
    }
